...
#define I2C_MUTEX_LOCK() \
    do                   \
    {                    \
    } while (xSemaphoreTake(i2c->lock, portMAX_DELAY) != pdPASS)
...
static void i2cDumpDqData(i2c_t *i2c)
{
    ...
    { // find maximum number of len decimal digits for formatting
        if (i2c->dq[a].length > lenDigits)
            lenDigits = i2c->dq[a].length;
        a++;
    }
    a = 0;
    while (lenDigits > 0)
    ...
    { // actual data
        do
        {
            ...
            { // address byte
                ...
                k += sprintf(&buf[k], "% 4X ", fifoBuffer[i]);
...
static void IRAM_ATTR fillCmdQueue(i2c_t *i2c, bool INTS)
{
    ...
    while (!needMoreCmds && (qp < i2c->queueCount))
    { // check if more possible cmds
        if (i2c->dq[qp].ctrl.stopCmdSent)
        { // marks that all required cmds[] have been added to peripheral
            qp++;
        }
        else
        {
    ...
    while (!done)
    { // fill the command[] until either 0..14 filled or out of cmds and last cmd is STOP
        //CMD START
        I2C_DATA_QUEUE_t *tdq = &i2c->dq[qp]; // simpler coding

        if ((!tdq->ctrl.startCmdSent) && (cmdIdx < 14))
        { // has this dq element's START command been added?
        ...
        //CMD WRITE ADDRESS
        if ((!done) && (tdq->ctrl.startCmdSent))
        { // have to leave room for continue(END), and START must have been sent!
        ...
        if ((!done) && (tdq->ctrl.addrCmdSent))
        {                        //room in command[] for at least One data (read/Write) cmd
            uint8_t blkSize = 0; // max is 255
            while ((tdq->cmdBytesNeeded > tdq->ctrl.mode) && (!done))
            {                                                                                         // more bytes needed and room in cmd queue, leave room for END
        ...
        if (cmdIdx == 15)
        { //need continuation, even if STOP is in 14, it will not matter
        ...
        {
            if (tdq->ctrl.stopCmdSent)
            { // this queue element has been completely added to command[] buffer
                qp++;
    ...
    if (INTS)
    { // don't want to prematurely enable fifo ints until ISR is ready to handle them.
        if (ena_rx)
        {
...
static void IRAM_ATTR fillTxFifo(i2c_t *i2c)
{
    ...
    while ((a < i2c->queueCount) && !full)
    {
        ...
        if (tdq->ctrl.addrSent < tdq->ctrl.addrReq)
        { // need to send address bytes
            if (tdq->ctrl.addrReq == 2)
            { //10bit
                if (tdq->ctrl.addrSent == 0)
                { //10bit highbyte needs sent
                    if (!full)
                    { // room in fifo
                ...
                if (tdq->ctrl.addrSent == 1)
                { //10bit Lowbyte needs sent
                    if (!full)
                    { // room in fifo
        ...
        { // write
            if (tdq->ctrl.addrSent == tdq->ctrl.addrReq)
            {                                                             //address has been sent, is Write Mode!
        ...
        { // read Queue Encountered, can't update QueuePos past this point, emptyRxfifo will do it
    ...
    if (a >= i2c->queueCount)
    { // disable TX IRQ, all tx Data has been queued
...
static void IRAM_ATTR emptyRxFifo(i2c_t *i2c)
{
    ...
    while ((moveCnt > 0) && (i2c->queuePos < i2c->queueCount))
    { // data to move

        I2C_DATA_QUEUE_t *tdq = &i2c->dq[i2c->queuePos]; //short cut

        while ((tdq->position >= tdq->length) && (i2c->queuePos < i2c->queueCount))
        { // find were to store
            i2c->queuePos++;
            tdq = &i2c->dq[i2c->queuePos];
        }

        if (i2c->queuePos >= i2c->queueCount)
        { // bad stuff, rx data but no place to put it!
        ...
        if (tdq->ctrl.mode == 1)
        { // read command
            if (moveCnt > (tdq->length - tdq->position))
            {   //make sure they go in this dq
 ...
 */
    ...
    i2c->errorByteCnt = 0;
    while (a < i2c->queueCount)
    { // add up all bytes loaded into fifo's
        tdq = &i2c->dq[a];
...
static void IRAM_ATTR i2c_isr_handler_default(void *arg)
{
    ...
    if (p_i2c->stage == I2C_DONE)
    { //get Out, can't service, not configured
    ...
    while (activeInt != 0)
    { // Ordering of 'if(activeInt)' statements is important, don't change
        ...
        if (activeInt & I2C_TXFIFO_EMPTY_INT_ST)
        {                      //should this be before Trans_start?
        ...
        if (activeInt)
        { // clear unhandled if possible?  What about Disabling interrupt?
...
static i2c_err_t i2cAddQueue(i2c_t *i2c, uint8_t mode, uint16_t i2cDeviceAddr, uint8_t *dataPtr, uint16_t dataLen, bool sendStop, bool dataOnly, EventGroupHandle_t event)
{
    ...
    if (event)
    {                                            // an eventGroup exist, so, initialize it
    ...
    { // expand
        ...
        { // bad stuff, unable to allocate more memory!
...
i2c_err_t i2cAddQueueRead(i2c_t *i2c, uint16_t i2cDeviceAddr, uint8_t *dataPtr, uint16_t dataLen, bool sendStop, EventGroupHandle_t event)
{
    ...
    if ((i2cDeviceAddr & 0xFC00) == 0x7800)
    { // ten bit read
...
i2c_err_t i2cProcQueue(i2c_t *i2c, uint32_t *readCount, uint16_t timeOutMillis)
{
    ...
    if (readCount)
    { //total reads accomplished in all queue elements
        *readCount = 0;
    }
    if (i2c == NULL)
    {
    ...
    { // need to push these address modes upstream, to AddQueue
    ...
    if (!i2c->intr_handle)
    { // create ISR for either peripheral
    ...
    if (i2c->exitCode != eBits)
    { // try to recover from O/S failure
        //  log_e("EventGroup Failed:%p!=%p",eBits,i2c->exitCode);
        eBits = i2c->exitCode;
    }
    if ((eBits & EVENT_ERROR) || (!(eBits & EVENT_DONE)))
    { // need accurate errorByteCnt for debug
        i2c_update_error_byte_cnt(i2c);
    }

    if (!(eBits == EVENT_DONE) && (eBits & ~(EVENT_ERROR_NAK | EVENT_ERROR_DATA_NAK | EVENT_ERROR | EVENT_DONE)))
    { // not only Done, therefore error, exclude ADDR NAK, DATA_NAK
    ...
    { // GROSS timeout, shutdown ISR , report Timeout
        i2c->stage = I2C_DONE;
        i2c->dev->int_ena.val = 0;
        i2c->dev->int_clr.val = 0x1FFF;
        i2c_update_error_byte_cnt(i2c);
        if ((i2c->errorByteCnt == 0) && (i2c->errorQueue == 0))
        { // Bus Busy no bytes Moved
        ...
        { // just a timeout, some data made it out or in.
            reason = I2C_ERROR_TIMEOUT;
    ...
    */
    ...
    while (b < i2c->queueCount)
    {
        ...
        if (b < i2c->queuePos)
        { // before any error
            if (i2c->dq[b].queueEvent)
            { // this data queue element has an EventGroup
                xEventGroupSetBits(i2c->dq[b].queueEvent, EVENT_DONE);
            }
        }
        else if (b == i2c->queuePos)
...
i2c_t *i2cInit(uint8_t i2c_num, int8_t sda, int8_t scl, uint32_t frequency) //before this is called, pins should be detached, else glitch
{
    ...
    if (frequency == 0)
    { // don't change existing frequency
...
i2c_err_t i2cWrite(i2c_t *i2c, uint16_t address, uint8_t *buff, uint16_t size, bool sendStop, uint16_t timeOutMillis)
{
    ...
    { //queued
        if (sendStop)
        { //now actually process the queued commands, including READs
            last_error = i2cProcQueue(i2c, NULL, timeOutMillis);
            if (last_error == I2C_ERROR_BUSY)
        ...
        { // stop not received, so wait for I2C stop,
